                ...
                <div slot="dropdown" class="dialog" style="z-index: 1000; min-width: 10rem;padding: .5rem 0;margin: .5rem 0 0;font-size: 1rem;color: #292b2c;text-align: left;list-style: none;background-color: #fff;-webkit-background-clip: padding-box;background-clip: padding-box;border: 1px solid rgba(0,0,0,.15);border-radius: .25rem;">
                    <div class="p-3" style="width: 400px;max-height: 1000px;overflow-y: scroll;"  >
                        <div class="form-group">
                            <v-select :on-change="selectKeep" :value.sync="temp.own" :options=" [{label:'Нет', value: 1}, {label:'Да', value: 2}]" placeholder="имеет группы?"></v-select>
                        </div>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                statuses: [{label:'Нет', value: 1}, {label:'Да', value: 2}],
            }
        ...
        methods: {
            ...
            setSelect()
            {
                if(this.filterData.keep) {
                    var teach = this.filterData.keep
                    this.temp.own = this.statuses[teach];
            ...
            selectKeep(val) {
                if (val)
                    this.filterData.keep = val.value
                this.temp.own = val;
            }
        }
