                    ...
                    <div class="p-3" style="width: 400px;max-height: 1000px;overflow-y: scroll;"  >
                        <div class="form-group">
                           <!-- <select class="form-control" v-model="form.teacher_id">
                                ...
                                <option :key="teacher.id" v-for="teacher in $common.data.teachersFullName" :value="teacher.id"> {{teacher.firstName}} {{teacher.secondName}}</option>
                            </select>-->
<!--
                    <div> {{ statuses }}</div>
-->
                            <v-select :on-change="selectTeacher" :value.sync="temp.teacher" :options="$common.data.teachersFullName" placeholder="Статус"></v-select>
                        </div>
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                statuses: this.$common.data.teachersFullName,
            }
        ...
        methods: {
            ...
            setSelect()
            {
                if(this.filterData.teacher_id) {
                    var teach = this.filterData.teacher_id
                    teach = teach -1;
                    this.temp.teacher = this.statuses[teach];
                    /*console.log(this.statuses[this.filterData.teacher_id]);
                    console.log(1);*/
            ...
            selectTeacher(val) {
                if (val)
                    this.filterData.teacher_id = val.value
